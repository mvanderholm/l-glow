...
- **Admin screens:** the combined Constitution/Tag Doshas screens are now four routes — `prakriti-questions.js`, `vikriti-questions.js`, `prakriti-tagging.js`, `vikriti-tagging.js` — each a thin wrapper (`table` + tier list as props) around one shared `ConstitutionEditor`/`DoshaTaggingScreen` component in `components/practitioner/`, so the editing/tagging logic itself isn't duplicated four ways even though the routes, nav entries, and tables are now fully independent.
...
- **Consumer-facing concept:** no quiz-taking UI exists yet for either assessment, so nothing changed in running code here — but the future consumer quiz UI should be scoped and built as two separate flows/entry points (distinct routes, distinct You-tab rows) rather than one combined "Constitution Assessment" with a mode switch, matching the split everywhere else. The open question of how Prakriti relates to `/quiz` and intake Section 14 (above) stays TBD, now understood as Prakriti-only — no longer entangled with Vikriti planning.
