...
- **Hamburger drawer** (`components/HamburgerDrawer.js`) now has a second section linking to every assessment (My Dosha, Agni, Guna, Tongue, Prakriti, Vikriti) plus a Playlist item — a second access point, not a replacement for You tab's "Your Assessments" section, which is untouched. No dedicated playlist screen exists and no dosha-specific Spotify URLs are populated yet (`data/content/music.js`'s `playlists` all have `url: null`), so "Playlist" opens Thea's general Spotify profile (`SPOTIFY_PROFILE_URL`) directly, same `Linking.openURL` pattern as "Book a Session" in the same file. Unlike the You tab rows, the drawer's assessment links don't branch to a saved result screen — they always start/retake the quiz, a deliberate simplification for what's meant to be a quick-access shortcut.
