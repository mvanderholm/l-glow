...
**Full audit, all 7 surfaces (6 assessments + intake) plus the User's Manual and daily check-in, read from the live code:** **[What Comes Back](https://claude.ai/code/artifact/4b6a4dc1-588f-456d-bd84-c289a0ee0dcb)**. Headline finding: mostly not an engineering problem — Guna, Agni, Tongue, and Dosha's archetype block are functionally complete and sitting on unreviewed draft copy, not missing features. But the audit surfaced two real bugs and one honesty gap, distinct from the content backlog:
...
**Not yet touched — the two "needs a call" items, held for a direct conversation rather than decided here:**
- Agni's closing-note content field (`lGlowNote`) is `null` for all four types, so that card never renders for anyone — different from a labeled DRAFT since nothing signals it's missing; worth flagging to Thea specifically alongside the general Agni-copy review.
