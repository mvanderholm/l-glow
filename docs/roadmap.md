...
**Built so far:**
- `constitution_questions` Supabase table — one shared table across all six tiers (`assessment` + `tier` columns), not six separate tables, since they're structurally identical. `options` is jsonb, not flattened columns, because each option's dosha tag is variable-length — one dosha (`["vata"]`) or a blend (`["vata","kapha"]`), a real capability Matt asked for since some traits genuinely sit between two doshas rather than purely belonging to one.
...
- Practitioner admin editor (`app/practitioner/constitution-questions.js`, "Constitution" in the hub nav) — assessment/tier picker, full question CRUD, and per-option Vata/Pitta/Kapha chip tagging.
- **Dosha Tagging screen** (`app/practitioner/dosha-tagging.js`, "Tag Doshas" in the hub nav, July 18 2026) — a faster companion to Constitution built specifically for the actual tagging pass across all six tiers now that content is loaded: browse every question in a tier, tap dosha chips directly with no edit-mode ceremony, each tap saves immediately. A progress count ("X of Y options tagged") and an "untagged only" filter help track a full tagging pass. Fixing prompt text, sections, or the option list itself still happens in Constitution — this screen only ever touches the `dosha` array. Explicitly doesn't try to auto-detect which options are meant to stay untagged by design (Vikriti Level 2/3's per-question catch-alls, Vikriti Level 1's "balanced" reading) — no stored flag distinguishes that from "not yet tagged," so the progress count won't hit 100% even when a tier is fully and correctly tagged; a judgment call for whoever's tagging, same as when the content was authored.
...
**All six tiers across both assessments are now content-complete** (108 Prakriti + 66 Vikriti = 174 questions). Remaining work across the whole #52 feature is the dosha-tagging pass (via `dosha-tagging.js`) and building the consumer-facing quiz UI — not more content.
