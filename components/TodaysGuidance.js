...
// to the spec's 4 rows (Nourishment/Herbs/Movement/Lifestyle) — Agni, Daily
// Rhythms, and the intention recap are their own always-visible Today
// sections now, not nested in here (Matt's decision, see the nav-restructure
// plan). Default: every row collapsed.
...
export default function TodaysGuidance({ dosha }) {
  ...
  return (
    <View style={styles.wrap}>
      <Text style={[type.h2, { color: c.text, marginBottom: spacing.md }]}>Today's Guidance</Text>

      <Row title="Nourishment" open={expanded === 'nourishment'} onPress={() => toggle('nourishment')} styles={styles} type={type}>
        <Text style={[type.label, { color: c.sage }]}>Favor</Text>
        ...
        <Text style={[type.label, { color: c.terracotta }]}>Reduce</Text>
        <View style={{ marginTop: spacing.xs }}>
          {rec.foods.avoid.map(f => <Bullet key={f} dotColor={c.saffron} type={type}>{f}</Bullet>)}
        </View>
      </Row>

      <Row title="Herbs" open={expanded === 'herbs'} onPress={() => toggle('herbs')} styles={styles} type={type}>
      ...
      <Row title="Movement" open={expanded === 'movement'} onPress={() => toggle('movement')} styles={styles} type={type}>
      ...
      <Row title="Lifestyle" open={expanded === 'lifestyle'} onPress={() => toggle('lifestyle')} styles={styles} type={type} last>
...
function Row({ title, open, onPress, children, styles, type, last }) {
  return (
